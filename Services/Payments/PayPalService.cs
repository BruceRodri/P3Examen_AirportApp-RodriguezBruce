...
namespace P3Examen_AirportApp.Services.Payments;
...
public class PayPalService
{
    ...
    public async Task<PayPalOrderResult> CreateOrderAsync(decimal total, string reference)
    {
        ...
        {
            throw new InvalidOperationException($"PayPal respondió con error: {content}");
        }
    ...
    public async Task<PayPalCaptureResult> CaptureOrderAsync(string orderId)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            throw new InvalidOperationException($"PayPal respondió con error al capturar: {content}");
        }
